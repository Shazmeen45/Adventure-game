...
while (true) {
  let startMatch = await inquirer.prompt({
    ...
    choices: [
      ...
      { name: 'Increase Health', value: 'increaseHealth' },
      { name: 'Run for Life..', value: 'runForLife' }
    ]
  } as any);
  ...
  if (options === 'increaseHealth') increaseHealthFun();
  if (options === 'runForLife') runForLifeFun();
